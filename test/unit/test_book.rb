...
class TestBook < MiniTest::Unit::TestCase
  ...
  def test_reviews_method_fetches_reviews
    ...
    subject = Skooby::Book.new(id: 108)
    assert_equal ({ page: 1, results: expected_reviews }), subject.reviews
  end

  def test_reviews_method_pass_given_page_to_reviews_parser
    subject = Skooby::Book.new(id: 108)
    Skooby::ReviewsParser.any_instance.expects(:last_reviews).with(5)
    subject.reviews(page: 5)
  end

  def test_reviews_method_returns_a_paginated_structure
    subject = Skooby::Book.new(id: 108)
    Skooby::ReviewsParser.any_instance.stubs(:last_reviews).returns([])
    assert_equal ({ page: 5, results: [] }), subject.reviews(page: 5)
  end
end
